...
void deleteAllLists(SchoolYear *&pHead, Account *&pHeadAccStudent, Account *&pHeadAccStaff /*,Staff *&pHeadStaff*/)
{
    ...
    while (tmp != nullptr) { // xóa list điểm
    ...
    while (pTemp != nullptr) { // xóa hs trong class
    ...
    while (pHeadAccStaff != nullptr) { //xóa account staff
        Account *temp = pHeadAccStaff;
        pHeadAccStaff = pHeadAccStaff->next;
        delete temp;
    }

    while (pHeadAccStudent != nullptr) { //xóa account student
